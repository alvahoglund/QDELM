...
@testitem "Scrambling map algorithms agree" begin
    function make_test_system(; u_intra=1.0, t=1.0, t_so=1.0, u_inter=1.0)
        ...
        hams = QDELM.matrix_representation_hams(hamiltonians(qd_system.grids, pf), qd_system)
    ...
    @testset "large U_intra (large spectral radius)" begin
    ...
    @testset "large time, multiple times" begin
        qd_system, measurements, ψ_res, ham_total = make_test_system()
        test_algorithms_agree(qd_system, measurements, ψ_res, ham_total, [200.0, 1000.0])
    end

    @testset "large U_intra, multiple times" begin
